...
public:
	void initDefault(const KeyType type, const int count)
	{
		...
		mUnusedList.addOrGet(type).addRange(move(list));
	}
	void quit() override
	{
#ifdef WINDOWS
	...
	void newClassList(const KeyType type, Vector<ClassType*>& classList, const int dataCount)
	{
		...
		if (!mUnusedList.isEmpty())
		{
			...
			if (objSet != nullptr && !objSet->isEmpty())
			{
				FOR(dataCount)
				{
	...
	int getTotalCount(const KeyType key) const { return mTotalCount.get(key).second; }
	void dump() override
	{
