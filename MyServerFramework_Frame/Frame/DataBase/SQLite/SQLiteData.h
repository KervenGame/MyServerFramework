...
public:
	...
	{
		mParameters.add(index, new SQLiteTableParam<T>(&param));
	}
	template<typename RealType, typename T>
	void registeEnumParam(T& param, const byte index)
	{
		mParameters.add(index, new SQLiteTableParam<RealType>(&param));
	}
	template<typename RealType, typename T>
	void registeEnumListParam(T& param, const byte index)
	{
		mParameters.add(index, new SQLiteTableParam<Vector<RealType>>(&param));
	}
	virtual void cloneTo(SQLiteData* target) { target->mID = mID; }
public:
