...
public:
	...
	const HashMap<llong, Character*>& getCharacterList(const byte type) const { return mCharacterTypeList.get(type); }
	Character* getCharacter(const llong characterID) const { return mCharacterIDList.get(characterID); }
